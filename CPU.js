...
function CPU(name, memory, prog_str, prog_addr)
{
	...
	const DUMP_MEM = 1;			// Dump memory on load
	...
	*/
	...
	{
		main.log_console(`[${name}] Assemble\n`);
		//console.log(str);
	...
	function assemble_token(token, next)
	{
		...
		var found_inst = -1;
		for (var it in inst_table) 
		{
			if (token.toUpperCase() == inst_table[it].text)
			{
				found_inst = it;
			}
		}
	...
	{
	//	log("Add Inst    " + inst_table[inst].text + "  " + p1);
		
		// Add instruction
		add_byte(inst);
		
		if (inst_table[inst].size == 1) add_byte(p1);
		if (inst_table[inst].size == 2) add_word(p1);
	}
